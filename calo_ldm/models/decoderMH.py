...
class DecoderMH(pl.LightningModule):
    def __init__(self, *,
            ...
            layer_seg=None, # N, R, Z, A
            layer_seg_dim=-2, 
            ):

        super().__init__()
        # assert learn_R=False
        assert output_activation is not None
        if z_padding_strategy=="preprocess":
             # in preprocess pad case, the split will not work. need info to strip the pre-added pad layers
             raise NotImplementedError # the preprocess zpad not work for layer-wise norm.
        ...
        self.output_activation_classes=[]
        self.layer_seg=list(layer_seg) 
        self.layer_seg_dim=int(layer_seg_dim)
        for s in self.layer_seg:
            self.output_activation_classes.append(output_activation_class())
    ...
    def forward(self, x, cond=None):
        ...
        #     pix=self.output_activation_class(pix).float() # whhy float??
        
        split_layers = torch.split(pix, self.layer_seg, dim=self.layer_seg_dim)
        activated_layers = [l(i) for l,i in zip(self.output_activation_classes,split_layers)] # activation should bt flattened!
        output = torch.cat(activated_layers,self.layer_seg_dim)  # .float() ?
        
        out = {'pixels_U_pred': output}
        # if self.learn_R:
        #     R = self.R_layers(x) # (*, 1)
        #     R = R + self.R_trim
        #     out['R_pred'] = R
